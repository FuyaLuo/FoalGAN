...
palette = [128, 64, 128, 244, 35, 232, 70, 70, 70, 102, 102, 156, 190, 153, 153, 153, 153, 153, 250, 170, 30,
           220, 220, 0, 107, 142, 35, 152, 251, 152, 70, 130, 180, 220, 20, 60, 210, 170, 100, 0, 0, 142, 0, 0, 70,
           0, 60, 100, 0, 80, 100, 0, 0, 230, 119, 11, 32]
...
def colorize_mask2(pred_tensor):
    "For original input size [288 360]"
    if len(pred_tensor.shape) == 4:
        pred_tensor_resize = F.interpolate(pred_tensor, size=[288, 360], mode='bilinear', align_corners=True)
        # pred_tensor_resize = pred_tensor
        # print(pred_tensor.shape)
        sm = torch.nn.Softmax(dim = 1)
        pred_sm = sm(pred_tensor_resize)
        ...
        # print(mask_ori.shape)
        # print(Image.fromarray(mask_ori))
        mask = np.asarray(Image.fromarray(mask_ori).resize((360, 288), Image.NEAREST), dtype=np.uint8)
        # mask = np.asarray(mask_ori, dtype=np.uint8)
    # mask: numpy array of the mask
    new_mask = Image.fromarray(mask.astype(np.uint8)).convert('P')
    new_mask.putpalette(palette)
...
def visual_attmaps(input_img, input_attmaps):
    h, w = input_attmaps.size()
    attmaps_resize = F.interpolate(input_attmaps.expand(1, 1, h, w), size=[256, 256], mode='bilinear', align_corners=True)
